...
- For documentation-backed reference skills, store source snapshots in `references/` and prefer copying the upstream markdown payload over hand-transcribing rendered HTML.
- For GitLab Docs specifically, prefer the page-local `index.md` payload used by the docs site's `Copy for LLM` button. Preserve the upstream directory structure in `references/` because copied links may reference `_index.md` or sibling `.md` pages that need that tree to resolve locally.
...
- Reference executable skill resources relative to the active `SKILL.md`, such as `scripts/...`.
- In runnable examples, prefer executing from the skill directory with paths like `./scripts/...` rather than repository-root paths.
- If a runtime resolves skill resources automatically, document the canonical resource path relative to `SKILL.md` instead of a machine-specific absolute path.
- Do not traverse to sibling skills with paths like `../other-skill/scripts/...`; invoke that skill directly and use its own skill-local resource paths.
...
- Documentation-backed reference skills specify how snapshots are sourced and updated.
