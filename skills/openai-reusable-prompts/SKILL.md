...
- Representative fixtures, tests, and evaluation checks cover production prompt changes.

## API deployment guidance

- For new OpenAI API work, manage prompts in application code, not reusable prompt objects.
- Keep a small prompt-builder module near the feature it supports.
- Use typed function inputs or schemas for dynamic values.
- Pass generated `instructions` and `input` directly to the Responses API.
- Pin a specific production model snapshot and evaluate behavior before changing the prompt or model.
- Roll out changes through normal deployment controls; use feature flags or configuration for staged releases.
- Reusable prompt objects are deprecated: creation was de-emphasized on June 3, 2026, and `v1/prompts` is scheduled to shut down on November 30, 2026. Use the OpenAI migration guide only for existing prompt-object integrations.
...
- Keep prompts and prompt builders in source control and change them through code review.
- Pin a specific production model snapshot when reproducibility matters.
- Maintain representative fixtures, tests, and evaluation suites for prompt behavior.
- Version and deploy prompt changes with the application; use feature flags or configuration for staged rollouts.
- When this guidance is also packaged as an agent skill, version the skill independently of the API prompt.

## Deliverables

Produce:

1. Reusable prompt template text for a code-managed `instructions` or `developer` message (Procedure steps 1-4)
...
## Code-managed prompt template

Use this as the `instructions` value or `developer` message sent to the Responses API. Keep it in source control near the feature it supports.

For an existing saved prompt object, migrate it to code; do not create a new reusable prompt object.
...
- [ ] Examples are diverse and included when they materially improve reliability
...
- [ ] Prompt is code-managed, model snapshot is pinned when needed, and rollout intent is defined
...
- Do not duplicate full skill procedures inside reusable developer instructions.
