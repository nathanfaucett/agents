...
- `name` **cannot contain** "anthropic" or "claude".
- `description` is **required**. Max 1024 chars. Write in **third person**. Must describe both what it does AND when to trigger it (Claude uses this field to decide when to invoke).
- `compatibility` is optional. Max 500 chars. List runtime requirements (e.g., "Requires glab CLI, jq, kubectl").
- `metadata.internal: true` hides the skill from the public skills.sh listing.
- `allowed-tools` is space-delimited (e.g., `Bash Read Glob`), not comma-separated.
